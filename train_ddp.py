...
from utils.utils import PSNR, SSIM, validation_ddp, validation_gpu, synthetic_loss
...
net = RecWeather().to( device )  # GPU or CPU
...
with torch_distributed_zero_first( local_rank = local_rank ):
    ...
    # perceptnet = vgg16(weights=torchvision.models.VGG16_Weights.IMAGENET1K_V1).features[:16]
    ...
    perceptual_loss_network = LossNetwork( perceptnet ).to( device )
    ...
    if pretrained:
        ...
        try:
            ...
            if is_main_process( local_rank ):  # only master process need calculate old psnr
                ...
                val_data_loader_pretrained = DataLoader(
                ...
                old_val_loss, old_val_psnr, old_val_ssim = validation_gpu(
                    net, val_data_loader_pretrained, device = device,
                    perceptual_loss_network = perceptual_loss_network,
                    ssim = ssim, psnr = psnr,
                    alpha = alpha_loss, beta = beta_loss, gamma = gamma_loss
                )
                del val_data_loader_pretrained  # only master processing
...
for epoch in range( epoch_start, num_epochs ):  # default epoch_start = 0
    ...
    # 如果不调用set_epoch, 那么每个epoch都会使用第 1个epoch 对应的 indices, 因为 epoch数没有改变(内部写死为1), 随机种子seed一直是初始状态
    # -------------------------------------------------------------------------------------------------------------
        ...
        if isapex:
            with torch.autocast( device_type = 'cuda', dtype = torch.float16, enabled = use_amp ):
                ...
                loss = synthetic_loss(
                    ...
                    alpha_loss, beta_loss, gamma_loss
                )
